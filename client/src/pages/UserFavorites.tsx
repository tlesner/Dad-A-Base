...
return (
	<div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', padding: '2rem' }}>
		<h2 style={{ textAlign: 'center' }}>
			Viewing {userParam ? `${user.username}'s` : 'your'} favorite videos
		</h2>

		<div style={{ display: 'flex', justifyContent: 'center', gap: '2rem', flexWrap: 'wrap' }}>
			{/* Left column: Favorite Videos */}
			<div style={{ flex: '1 1 200px', backgroundColor: '#f3f3f3', padding: '1rem', minWidth: '200px' }}>
				<h3>Favorite Videos</h3>
				<ul>
					{filteredVideos.map((video: any) => (
						<li key={video.videoId} style={{ cursor: 'pointer' }} onClick={() => handleOpenModal(video)}>
							{video.title}
						</li>
					))}
				</ul>
			...
			<div style={{ flex: '2 1 300px', minWidth: '300px' }}>
				...
				<div style={{ backgroundColor: '#f3f3f3', padding: '1rem', textAlign: 'center' }}>
					...
					<div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '0.5rem' }}>
						{tags.map((tag) => (
							<button
								...
								style={{
									padding: '0.5rem 1rem',
									border: selectedTag === tag ? '2px solid #000' : '1px solid #ccc',
									backgroundColor: selectedTag === tag ? '#ddd' : '#fff',
									borderRadius: '4px',
									cursor: 'pointer',
								}}>
								{tag}
